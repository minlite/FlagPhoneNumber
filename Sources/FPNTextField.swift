...
open class FPNTextField: UITextField, UITextFieldDelegate {
    ...
    private var leftViewSize: CGSize {
        let width = (manualPhoneCode ? 0 : flagButtonSize.width) + getWidth(text: phoneCodeTextField.text!) + 8
    ...
    private func setupLeftView() {
        ...
        if !manualPhoneCode {
            ...
            NSLayoutConstraint(item: flagButton, attribute: .leading, relatedBy: .equal, toItem: leftView, attribute: .leading, multiplier: 1, constant: 8).isActive = true
            ...
            NSLayoutConstraint(item: phoneCodeTextField, attribute: .leading, relatedBy: .equal, toItem: leftView, attribute: .leading, multiplier: 1, constant: 8).isActive = true
        }
    ...
    // Private
    @objc private func didEditCountryCode() {
        selectedCountry = countryRepository.getCountry(with: phoneCodeTextField.text ?? "+1")
        setNeedsLayout()
